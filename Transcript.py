...
class Transcript:
    ...
    def __init__(self, id, strand=None):
        self.cachedTranscriptRawExons = None
        if type(id) != EssexNode:  # not an EssexNode
            ...
            if changeNode is not None:
                ...
                if len(changeString) > 0:
                    self.structureChange = changeString
            exons = self.exons
            UTR = self.UTR
    ...
    def equals(self, other):
        if self.getSource() != other.getSource():
            return False
        if self.getStrand() != other.getStrand():
            return False
        ...
        for i in range(m):
            ...
            if thisExon.getBegin() != thatExon.getBegin():
                return False
            if thisExon.getEnd() != thatExon.getEnd():
                return False
        return True
    ...
    def overlaps(self, *parms):
        numParms = len(parms)
        if numParms < 1 or numParms > 2:
            raise Exception("1 or 2 parms expected")
        if numParms == 2:
            [begin, end] = parms
            return self.begin < end and begin < self.end
        otherTranscript = parms[0]
        return self.begin < otherTranscript.end and otherTranscript.begin < self.end
    ...
    def setUTRtypes(self):
        ...
        if strand == "+":
            for utr in UTR:
                begin = utr.getBegin()
                if begin < CDSbegin:
                    utr.setType("five_prime_UTR")
                else:
                    utr.setType("three_prime_UTR")
        else:
            for utr in UTR:
                begin = utr.getBegin()
                if begin > CDSbegin:
    ...
    def deleteExonRef(self, victim):
        ...
        if i >= numExons:
            raise Exception(
                "Can't find exon " + victim + " in Transcript::deleteExon()"
            )
        self.deleteExon(i)
    ...
    def toGff(self):
        ...
        if begin is not None:
            ...
            strand = self.strand
            extra = ""
            if re.search("\S", extraFields):
                extra = "; " + extraFields
            change = self.structureChange
            if change:
                extra += '; structure_change "' + change + '"'
            score = self.score
            if score is None:
                score = "."
            gff += (
        ...
        if not stopCodonFound:
            ...
            if strand == "+":
                ...
                if stopCodon != "TAG" and stopCodon != "TAA" and stopCodon != "TGA":
                    print(
                        "Warning!  No stop codon found for",
                        self.transcriptId,
                        self.strand,
    ...
    def isPartial(self):
        ...
        numExons = len(exons)
        if numExons == 1:
            return exons[0].getType() != "single-exon"
        return (
            exons[0].getType() != "initial-exon"
            or exons[numExons - 1].getType() != "final-exon"
        )
        ...
        for i in range(numExons):
            ...
            if transcriptCoord < exonLen:
                if self.getStrand() == "+":
                    ...
                    return exon.getEnd() - transcriptCoord - 1
            transcriptCoord -= exonLen
        id = self.getID()
        raise Exception(
            "coordinate is beyond transcript end: " + original + " (" + id + ")"
        )

    def mapToTranscript(self, genomicCoord):
        if self.cachedTranscriptRawExons is None:
            self.cachedTranscriptRawExons = self.getRawExons()
        exons = self.cachedTranscriptRawExons
        transcriptCoord = 0
        for exon in exons:
            if exon.containsCoordinate(genomicCoord):
                if self.getStrand() == "+":
                    ...
                    return transcriptCoord + exon.getEnd() - genomicCoord - 1
            exonLen = exon.getLength()
            transcriptCoord += exonLen
        return -1
    ...
    def getRawExons(self):
        ...
        if not rawExons or len(rawExons) == 0:
            ...
            for exon in exons:
                if exon.getLength() > 0:
                    rawExons.append(exon.copy())
            for utr in UTR:
                if utr.getLength() > 0:
                    rawExons.append(utr.copy())
        ...
        while i < n:
            ...
            if i + 1 < n:
                ...
                if (
                    exon.getEnd() == nextExon.getBegin()
                    or exon.getEnd() == nextExon.getBegin() - 1
                    or exon.overlaps(nextExon)
                ):
